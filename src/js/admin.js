...
window.AdminApp = {
  ...
  setupEventListeners: function(instance) {
    ...
    $("#createElection").click(async function(event) {
      ...
      try {
        ...
        const startDay = parseInt(document.getElementById('startDateDay').value);
        const startMonth = parseInt(document.getElementById('startDateMonth').value);
        const startYear = parseInt(document.getElementById('startDateYear').value);
        const startHour = parseInt(document.getElementById('startDateHour').value);
        const startMinute = parseInt(document.getElementById('startDateMinute').value);
        
        const endDay = parseInt(document.getElementById('endDateDay').value);
        const endMonth = parseInt(document.getElementById('endDateMonth').value);
        const endYear = parseInt(document.getElementById('endDateYear').value);
        const endHour = parseInt(document.getElementById('endDateHour').value);
        const endMinute = parseInt(document.getElementById('endDateMinute').value);
        
        // Format dates
        // const startDateFormatted = `${startDay}/${startMonth}/${startYear} ${startHour}:${startMinute}`;
        // const endDateFormatted = `${endDay}/${endMonth}/${endYear} ${endHour}:${endMinute}`;

        // const startDateFormatted = new Date(`${startYear}-${startMonth}-${startDay}T${startHour}:${startMinute}:00`.replace(/-/g, '\/').replace(/T.+/, ''));
        // const endDateFormatted = new Date(`${endYear}-${endMonth}-${endDay}T${endHour}:${endMinute}:00`.replace(/-/g, '\/').replace(/T.+/, ''));;

        const startDateFormatted = new Date(startYear, startMonth - 1, startDay, startHour, startMinute);
        const endDateFormatted = new Date(endYear, endMonth - 1, endDay, endHour, endMinute);

        const startDate = startDateFormatted.getTime() / 1000;
        const endDate = endDateFormatted.getTime() / 1000;

        // Validate dates
        const now = Math.floor(Date.now() / 1000);

        console.log("Date: ", new Date(startDate * 1000).toString(), new Date(now * 1000).toString());
        console.log("Date in miliseconds: ", startDate, now);
        console.log("Difference: ", now - startDate);
        console.log("Formatted: ", startDateFormatted);
